...
from fourier2d import FourierTransform2D as ft2d
...
class Frank2D(object):
    ...
    def set_kernel(self, type_kernel, kernel_params):
        ...
        if type_kernel == 'SquareExponential':
            self._kernel =  SquareExponential(self._Nx, self._Rmax, kernel_params, self._FT).kernel_radial
    ...
    def set_fit_method(self, u, v, Vis, Weights, type_kernel, kernel_params, method, rtol):
    ...
    def preprocess_vis(self, u, v, Vis, Weights):
        if not self._set_gridded_data:
            ...
            grid = Gridding(self._Nx, self._Rmax, self._FT)
    ...
    def fit(self, u, v, Vis, Weights, type_kernel = 'SquareExponential', kernel_params = [-5, 60, 1e4],
        ...
        self.preprocess_vis(u_deproj, v_deproj, Vis_deproj, Weights)

        # Frank1D guess.
        if frank1d_guess:
            print("Setting guess with Frank1D ...")
            self.set_guess(self.frank1d_solution(u, v, Vis, Weights))

        print("Setting fit with " + method + " ...")
        self.set_fit_method(u, v, Vis, Weights, type_kernel, kernel_params, method, rtol)
    ...
    def frank1d_solution(self, u, v, Vis, Weights, alpha = 1.3, w_smooth = 1e-1, n_pts = 300):
        inc, pa, dra, ddec = self._geometry
